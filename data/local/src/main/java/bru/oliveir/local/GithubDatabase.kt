...
abstract class GithubDatabase : RoomDatabase() {
    ...
    companion object {
        fun getInstance(context: Context) =
            Room.databaseBuilder(context.applicationContext, GithubDatabase::class.java, "Github.db")
        ...
        private val MIGRATION_1_2 = object : Migration(1, 2) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL("CREATE TABLE IF NOT EXISTS `Pull` (`pullId` INTEGER NOT NULL, `title` TEXT NOT NULL, `body` TEXT NOT NULL, `repositoryId` INTEGER NOT NULL, `userId` INTEGER NOT NULL, PRIMARY KEY(`pullId`), FOREIGN KEY(`repositoryId`) REFERENCES `Repository`(`repositoryId`) ON UPDATE NO ACTION ON DELETE NO ACTION )")
